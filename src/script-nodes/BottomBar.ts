...
class BottomBar extends ScriptNode {
	...
	awake() {
		...
		const upNumber = this.parent.getByName("upNumber");
		...
		const flashNumber = this.parent.getByName("flashNumber");
	...
	update() {
		const timeDisplay = this.parent.getByName("timeDisplay");
		timeDisplay.text = _root.clock_display2;
		checkTime();

		const fps = this.scene.game.loop.actualFps
		const fpsCounter = this.parent.getByName("fpsCounter");
		fpsCounter.text = Math.ceil(fps);
		_root.fpsnoround = fps;
		_root.fps = Math.round(fps);
		if (fps > 30) {
			fpsCounter.setStyle({ "color": "#00ff00" });
		} else if (fps > 20) {
			fpsCounter.setStyle({ "color": "#ffff00" });
		} else {
			fpsCounter.setStyle({ "color": "#ff0000" });
		}
	}
	/* END-USER-CODE */
}
